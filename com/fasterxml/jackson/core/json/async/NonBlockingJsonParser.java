...
public class NonBlockingJsonParser extends NonBlockingJsonParserBase implements ByteArrayFeeder
{
    ...
    public void feedInput(final byte[] buf, final int start, final int end) throws IOException {
        if (this._inputPtr < this._inputEnd) {
            this._reportError("Still have %d undecoded bytes, should not call 'feedInput'", (Object)(this._inputEnd - this._inputPtr));
        }
        if (end < start) {
            this._reportError("Input end (%d) may not be before start (%d)", (Object)end, (Object)start);
        }
        if (this._endOfInput) {
    ...
    private final JsonToken _finishBOM(int bytesHandled) throws IOException {
        while (this._inputPtr < this._inputEnd) {
            ...
            switch (bytesHandled) {
                ...
                case 2: {
                    if (ch != 191) {
                        this._reportError("Unexpected byte 0x%02x following 0xEF 0xBB; should get 0xBF as third byte of UTF-8 BOM", (Object)ch);
                        break;
                    }
                    break;
                }
                case 1: {
                    if (ch != 187) {
                        this._reportError("Unexpected byte 0x%02x following 0xEF; should get 0xBB as second byte UTF-8 BOM", (Object)ch);
                        break;
                    }
                    break;
                }
    ...
    protected JsonToken _reportErrorToken(final String actualToken) throws IOException {
        this._reportError("Unrecognized token '%s': was expecting %s", (Object)this._textBuffer.contentsAsString(), (Object)this._validJsonTokenList());
        return JsonToken.NOT_AVAILABLE;
    }
    ...
    private JsonToken _handleOddName(final int ch) throws IOException {
        ...
        if ((this._features & NonBlockingJsonParser.FEAT_MASK_ALLOW_UNQUOTED_NAMES) == 0x0) {
            ...
            this._reportUnexpectedChar((int)c, "was expecting double-quote to start field name");
        }
