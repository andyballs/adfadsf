...
public class HtmlFoldParser implements FoldParser
{
    ...
    private int getTagCloseInfo(final Token tagNameToken, final RSyntaxTextArea textArea, int line, final TagCloseInfo info) {
        info.reset();
    ...
    private static boolean isEndOfLastFold(final Stack<String> tagNameStack, final Token tagNameToken) {
        return tagNameToken != null && tagNameToken.getLexeme() != null && !tagNameStack.isEmpty() && tagNameToken.getLexeme().equalsIgnoreCase(tagNameStack.peek());
    }
    
    private static boolean isFoldableTag(final Token tagNameToken) {
        return tagNameToken != null && tagNameToken.getLexeme() != null && HtmlFoldParser.FOLDABLE_TAGS.contains(tagNameToken.getLexeme().toLowerCase());
    }
