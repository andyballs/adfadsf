...
public class RtfGenerator
{
    ...
    public RtfGenerator(final Color mainBG) {
        this.mainBG = mainBG;
    ...
    private int fixFontSize(float pointSize) {
        if (this.screenRes != 72) {
            pointSize = (float)Math.round(pointSize * 72.0f / this.screenRes);
        }
    ...
    public String getRtf() {
        final int mainBGIndex = getColorIndex(this.colorList, this.mainBG);
        final StringBuilder sb = new StringBuilder();
        sb.append("{");
        ...
        final int bgIndex = mainBGIndex + 1;
