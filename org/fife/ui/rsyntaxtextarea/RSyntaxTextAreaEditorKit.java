...
public class RSyntaxTextAreaEditorKit extends RTextAreaEditorKit
{
    ...
    public IconRowHeader createIconRowHeader(final RTextArea textArea) {
        return (IconRowHeader)new FoldingAwareIconRowHeader((RSyntaxTextArea)textArea);
    }
    ...
    static {
        ...
        defaultActions = new Action[] { new CloseCurlyBraceAction(), new CloseMarkupTagAction(), new BeginWordAction("caret-begin-word", false), new BeginWordAction("selection-begin-word", true), new ChangeFoldStateAction("RSTA.CollapseFoldAction", true), new ChangeFoldStateAction("RSTA.ExpandFoldAction", false), new CollapseAllFoldsAction(), new CopyAsStyledTextAction(), new DecreaseIndentAction(), new DeletePrevWordAction(), new DumbCompleteWordAction(), new EndAction("caret-end", false), new EndAction("selection-end", true), new EndWordAction("caret-end-word", false), new EndWordAction("caret-end-word", true), new ExpandAllFoldsAction(), new GoToMatchingBracketAction(), new InsertBreakAction(), new InsertTabAction(), new NextWordAction("caret-next-word", false), new NextWordAction("selection-next-word", true), new PossiblyInsertTemplateAction(), new PreviousWordAction("caret-previous-word", false), new PreviousWordAction("selection-previous-word", true), new SelectWordAction(), new ToggleCommentAction() };
    }
            ...
            try {
                ...
                if (alignCurlyBraces) {
                    ...
                    if (this.bracketInfo.y > -1) {
                        try {
                            final String ws = RSyntaxUtilities.getLeadingWhitespace((Document)doc, this.bracketInfo.y);
    ...
    public static class CopyAsStyledTextAction extends RecordableTextAction
    {
        ...
        public CopyAsStyledTextAction(final String themeName, final Theme theme) {
            super("RSTA.CopyAsStyledTextAction_" + themeName);
            this.theme = theme;
        }
        
        public CopyAsStyledTextAction(final String name, final Icon icon, final String desc, final Integer mnemonic, final KeyStroke accelerator) {
            super(name, icon, desc, mnemonic, accelerator);
        }
    ...
    public static class DecreaseIndentAction extends RecordableTextAction
    {
        ...
        private void handleDecreaseIndent(final Element elem, final Document doc, final int tabSize) throws BadLocationException {
            final int start = elem.getStartOffset();
    ...
    public static class DumbCompleteWordAction extends RTextAreaEditorKit.DumbCompleteWordAction
    {
        ...
        protected int getPreviousWord(final RTextArea textArea, int offs) throws BadLocationException {
            ...
            int prevWordStart;
            for (prevWordStart = this.getPreviousWordStartInLine(doc, elem, offs); prevWordStart == -1 && line > 0; --line, elem = root.getElement(line), prevWordStart = this.getPreviousWordStartInLine(doc, elem, elem.getEndOffset())) {}
            return prevWordStart;
        }
        
        private int getPreviousWordStartInLine(final RSyntaxDocument doc, final Element elem, final int offs) throws BadLocationException {
            int start;
            int cur;
            for (start = elem.getStartOffset(), cur = offs; cur >= start; --cur) {
        ...
        protected int getWordEnd(final RTextArea textArea, final int offs) throws BadLocationException {
            ...
            int end;
            int wordEnd;
            for (end = elem.getEndOffset() - 1, wordEnd = offs; wordEnd <= end && isIdentifierChar(doc.charAt(wordEnd)); ++wordEnd) {}
            return wordEnd;
        }
        ...
        private static int getWordStartImpl(final RSyntaxDocument doc, final Element elem, final int offs) throws BadLocationException {
            int start;
            int wordStart;
            for (start = elem.getStartOffset(), wordStart = offs; wordStart >= start; --wordStart) {
    ...
    public static class ExpandAllFoldsAction extends FoldRelatedAction
    {
        ...
        public void actionPerformedImpl(final ActionEvent e, final RTextArea textArea) {
            ...
            if (rsta.isCodeFoldingEnabled()) {
                ...
                RSyntaxUtilities.possiblyRepaintGutter((RTextArea)rsta);
            }
            else {
                UIManager.getLookAndFeel().provideErrorFeedback((Component)rsta);
            }
    ...
    abstract static class FoldRelatedAction extends RecordableTextAction
    {
        ...
        FoldRelatedAction(final String name, final Icon icon, final String desc, final Integer mnemonic, final KeyStroke accelerator) {
            super(name, icon, desc, mnemonic, accelerator);
        }
        ...
        protected int getNextWord(final RTextArea textArea, int offs) throws BadLocationException {
            ...
            if (offs != end) {
                ...
                if (doc.isIdentifierChar(languageIndex, ch)) {
                    ...
                    do {
                        ch = this.seg.next();
                    } while (ch != '\uffff' && !doc.isIdentifierChar(languageIndex, ch) && !Character.isWhitespace(ch));
                }
                while (Character.isWhitespace(ch)) {
                    ch = this.seg.next();
                }
                ...
                if (doc != null) {
                    try {
                        ...
                        if (template != null) {
                            ...
                            this.doDefaultInsert((RTextArea)rsta);
                        }
                    }
                    catch (BadLocationException ble) {
                        UIManager.getLookAndFeel().provideErrorFeedback(textArea);
    ...
    public static class ToggleCommentAction extends RecordableTextAction
    {
        ...
        public void actionPerformedImpl(final ActionEvent e, final RTextArea textArea) {
            ...
            textArea.beginAtomicEdit();
            try {
                final boolean add = this.getDoAdd((Document)doc, map, start, end, startEnd);
                for (line1 = start; line1 <= end; ++line1) {
                    final Element elem2 = map.getElement(line1);
                    this.handleToggleComment(elem2, (Document)doc, startEnd, add);
                }
            }
            catch (BadLocationException ble) {
                ble.printStackTrace();
