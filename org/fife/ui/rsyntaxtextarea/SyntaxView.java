...
public class SyntaxView extends View implements TabExpander, TokenOrientedView, RSTAView
{
    ...
    protected void damageLineRange(final int line0, final int line1, final Shape a, final Component host) {
        if (a != null) {
            ...
            final Rectangle area2 = this.lineToRect(a, line1);
            if (area0 != null && area2 != null) {
                final Rectangle dmg = area0.union(area2);
    ...
    private float drawLine(final TokenPainter painter, Token token, final Graphics2D g, final float x, final float y, final int line) {
        float nextX = x;
        for (boolean paintBG = this.host.getPaintTokenBackgrounds(line, y); token != null && token.isPaintable() && nextX < this.clipEnd; nextX = painter.paint(token, g, nextX, y, this.host, this, (float)this.clipStart, paintBG), token = token.getNextToken()) {}
        ...
        while (token != null && token.isPaintable() && nextX < this.clipEnd) {
            if (token.containsPosition(selStart)) {
                ...
                if (selCount == tokenLen) {
                    nextX = painter.paintSelected(token, g, nextX, y, this.host, this, (float)this.clipStart, useSTC);
                }
                else {
                    this.tempToken.copyFrom(token);
                    ...
                    nextX = painter.paint(token, g, nextX, y, this.host, this, (float)this.clipStart);
                }
            }
    ...
    private float getLineWidth(final int lineNumber) {
        ...
        return RSyntaxUtilities.getTokenListWidth(tokenList, (RSyntaxTextArea)this.getContainer(), (TabExpander)this);
    }
    ...
    public int getNextVisualPositionFrom(final int pos, final Position.Bias b, final Shape a, final int direction, final Position.Bias[] biasRet) throws BadLocationException {
        return RSyntaxUtilities.getNextVisualPositionFrom(pos, b, a, direction, biasRet, (View)this);
    }
    ...
    public void paint(final Graphics g, final Shape a) {
        ...
        for (int line = linesAbove; y < clip.y + clip.height + this.ascent && line < lineCount; y += this.lineHeight, ++line) {
            ...
            h.paintLayeredHighlights((Graphics)g2d, startOffset, endOffset, a, (JTextComponent)this.host, (View)this);
            ...
            h.paintParserHighlights((Graphics)g2d, startOffset, endOffset, a, (JTextComponent)this.host, (View)this);
    ...
    public int viewToModel(final float fx, final float fy, final Shape a, final Position.Bias[] bias) {
        ...
        final Token tokenList = doc.getTokenListForLine(lineIndex);
        this.tabBase = alloc.x;
        final int offs = tokenList.getListOffset((RSyntaxTextArea)this.getContainer(), this, (float)this.tabBase, (float)x);
