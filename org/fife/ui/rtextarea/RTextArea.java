...
public class RTextArea extends RTextAreaBase implements Printable
{
    ...
    protected JPopupMenu createPopupMenu() {
        ...
        menu.add(this.undoMenuItem = this.createPopupMenuItem((Action)RTextArea.undoAction));
        menu.add(this.redoMenuItem = this.createPopupMenuItem((Action)RTextArea.redoAction));
        menu.addSeparator();
        menu.add(this.cutMenuItem = this.createPopupMenuItem((Action)RTextArea.cutAction));
        menu.add(this.createPopupMenuItem((Action)RTextArea.copyAction));
        menu.add(this.pasteMenuItem = this.createPopupMenuItem((Action)RTextArea.pasteAction));
        menu.add(this.deleteMenuItem = this.createPopupMenuItem((Action)RTextArea.deleteAction));
        menu.addSeparator();
        menu.add(this.createPopupMenuItem((Action)RTextArea.selectAllAction));
        return menu;
    }
    
    private static void createPopupMenuActions() {
        final int mod = getDefaultModifier();
        ...
        if (h != null) {
            if (ranges != null) {
                for (final DocumentRange range : ranges) {
                    try {
                        h.addMarkAllHighlight(range.getStartOffset(), range.getEndOffset(), (Highlighter.HighlightPainter)this.markAllHighlightPainter);
                    }
    ...
    public synchronized void playbackLastMacro() {
        if (RTextArea.currentMacro != null) {
            final List<Macro.MacroRecord> macroRecords = (List<Macro.MacroRecord>)RTextArea.currentMacro.getMacroRecords();
                ...
                try {
                    for (final Macro.MacroRecord record : macroRecords) {
                        for (final Action action : actions) {
                            if (action instanceof RecordableTextAction && record.id.equals(((RecordableTextAction)action).getMacroID())) {
    ...
    public int print(final Graphics g, final PageFormat pageFormat, final int pageIndex) {
        return RPrintUtilities.printDocumentWordWrap(g, (JTextComponent)this, this.getFont(), pageIndex, pageFormat, this.getTabSize());
    }
    ...
    private String replaceTabsWithSpaces(final String text, final int docOffs, final int firstTab) throws BadLocationException {
    ...
    public static void setActionProperties(final int action, final String name, final char mnemonic, final KeyStroke accelerator) {
        setActionProperties(action, name, (int)mnemonic, accelerator);
    }
    ...
    public static void setActionProperties(final int action, final String name, final Integer mnemonic, final KeyStroke accelerator) {
        ...
        switch (action) {
            case 1: {
                tempAction = (Action)RTextArea.cutAction;
                break;
            }
            case 0: {
                tempAction = (Action)RTextArea.copyAction;
                break;
            }
            case 3: {
                tempAction = (Action)RTextArea.pasteAction;
                break;
            }
            case 2: {
                tempAction = (Action)RTextArea.deleteAction;
                break;
            }
            case 5: {
                tempAction = (Action)RTextArea.selectAllAction;
                break;
            }
            default: {
                return;
    ...
    public static synchronized void setIconGroup(final IconGroup group) {
        ...
        RTextArea.cutAction.putValue("SmallIcon", (Object)icon);
        icon = group.getIcon("copy");
        RTextArea.copyAction.putValue("SmallIcon", (Object)icon);
        icon = group.getIcon("paste");
        RTextArea.pasteAction.putValue("SmallIcon", (Object)icon);
        icon = group.getIcon("delete");
        RTextArea.deleteAction.putValue("SmallIcon", (Object)icon);
        icon = group.getIcon("undo");
        RTextArea.undoAction.putValue("SmallIcon", (Object)icon);
        icon = group.getIcon("redo");
        RTextArea.redoAction.putValue("SmallIcon", (Object)icon);
        icon = group.getIcon("selectall");
        RTextArea.selectAllAction.putValue("SmallIcon", (Object)icon);
    ...
    public void setMarkAllOnOccurrenceSearches(final boolean markAll) {
        if (markAll != this.markAllOnOccurrenceSearches) {
            ...
            this.firePropertyChange("RTA.markAllOnOccurrenceSearches", !markAll, markAll);
        }
    }
