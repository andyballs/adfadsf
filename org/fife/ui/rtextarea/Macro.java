...
public class Macro
{
    ...
    public Macro(final File file) throws IOException {
        ...
        try {
            ...
            final InputSource is = new InputSource((Reader)new UnicodeReader((InputStream)new FileInputStream(file), "UTF-8"));
    ...
    public Macro(final String name, final List<MacroRecord> records) {
        this.name = name;
        if (records != null) {
            (this.macroRecords = new ArrayList<MacroRecord>(records.size())).addAll(records);
        }
        else {
