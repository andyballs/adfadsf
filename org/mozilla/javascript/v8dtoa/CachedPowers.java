...
public class CachedPowers
{
    ...
    static int getCachedPower(final int e, final int alpha, final int gamma, final DiyFp c_mk) {
        final int kQ = 64;
        final double k = Math.ceil((alpha - e + kQ - 1) * 0.30102999566398114);
        final int index = (308 + (int)k - 1) / 8 + 1;
        final CachedPower cachedPower = CachedPowers.CACHED_POWERS[index];
    ...
    static class CachedPower
    {
        ...
        CachedPower(final long significand, final short binaryExponent, final short decimalExponent) {
